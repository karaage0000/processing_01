...
class Ball {
  ...
  void update() {
    ...
    latey = y0 +v0y * (t-0.01) + 0.5 * g * (t-0.01) * (t-0.01);
  ...
  if (mouseY >= height/2) {
    if (mouseX <= width/2) {
      ...
      if(mouseX <= width/2 && mouseX >= width/2 -300){
        ballpoint = 1;
      }else if(mouseX <= width/2-300 && mouseX >= width/2 -600){
        ballpoint = 2;
      }else{
        ballpoint = 3;
      }
...
class GOAL {
  ...
  void score() {
    fill(0);
    textSize(40);
    text("Score: " + point, width/2-100, 30);
  }
  void wall() {
    wallx = x1+100;
    wally = y1-250;
...
void wallattack() {
  if (ball.x >= goal.wallx && ball.x <= goal.wallx + 30) {
    if (ball.y >= goal.wally && ball.y <= goal.y1) {
      float v0x = -e*((xe -xs) / (te - ts)); //x方向の初速度を計算
      float v0y = -e*((ye - ys) / (te - ts)); //y方向の初速度を計算
...
void goalattack() {
  if (ball.x > goal.x1-100) {
    if (ball.y > goal.y1-30 && ball.y < goal.y1+50) {
      float v0x = -e * ((xe -xs) / (te - ts)); //x方向の初速度を計算
      float v0y = -e *((ye - ys) / (te - ts)); //y方向の初速度を計算
      ball = new Ball(ball.x, ball.y, v0x, v0y);
    }
...
void gamePlay() {
  fill(255);
  fill(255,0,0); rect(width/2-300, height/2, 300, height);
  fill(0,255,0); rect(width/2-600, height/2, 300, height);
  fill(0,0,255); rect(0, height/2, 150, height);
  ball.update();
  ball.show();
  goal.show();
  goal.update();
...
void ending() {
  ...
  fill(0);
  textSize(40);
  text("END!", width/2, 400);
  text("score:" + endpoint, width/2, 600);
  changeGameMode();
}
